...
lcd_wait_bf_clear:
    ; Preserve direction of last four bits of DDRB
    ; but toggle LCD data lines to input
    lda LCD_DDR                     ;1111111x
    and #(BLINK_PORT_MASK)          ;1111111x
    ora #(LCD_DDR_READ_MASK)        ;0000111x
    sta LCD_DDR                     ;0000111x
    ; Preserve status of blink led
@wait_loop:
    lda LCD_PORT                    ;
    and #(BLINK_PORT_MASK)          ;    
    ora #(LCD_READ_MODE | LCD_COMMAND_MODE) ;RS=0, RW=0
    sta LCD_PORT
    ora #(LCD_ENABLE_FLAG)
    sta LCD_PORT
    lda LCD_PORT
    ; Store result from LCD data lines
    sta lcd_temp_char1              ; read the result
    ; Toggle enable
    eor #(LCD_ENABLE_FLAG)
    sta LCD_PORT
    eor #(LCD_ENABLE_FLAG)
